...
from keyboards.default.buton import btn_menu, btn_pizza_list, back_basket, delivery_or_take_away, belissimo_branches, \
    share_location, share_contact, menu
from keyboards.inline.buton import plus_minus, product_cb, basket_buttons, item_cb
...
async def f_name(message: types.Message):
    ...
    image_name = ProductDB().revert_image(image_data, f'{BASE_DIR}/media/Somsa.jpg')
...
async def my_order(message: types.Message):
    ...
    if order:
        ...
        await message.answer_photo(photo=open("media/Somsa.jpg", "rb"),
